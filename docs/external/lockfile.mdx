...
*   Regularly update the lockfile via `bazel mod tidy` to reflect changes in
    project dependencies or configuration. This ensures that subsequent builds
    are as efficient and repeatable as possible. This command also formats
    module files and fixes supported `use_repo` calls.
