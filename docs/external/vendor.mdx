...
`bazel mod tidy` also evaluates all module extensions, including those used
only by dependencies. Run `bazel vendor` without target arguments to vendor
the full dependency graph before running `bazel mod tidy --nofetch` offline.
