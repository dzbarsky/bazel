  ...
  private record AnalysisPhaseFlags(
    ...
    static AnalysisPhaseFlags of(CoreOptions options) {
      return new AnalysisPhaseFlags(
          ...
          options.verboseVisibilityErrors,
          options.enforceTransitiveVisibility,
          options.checkTestonlyForOutputFiles);
    }

    BuildOptions toNoConfigOptions() {
      BuildOptions options = EMPTY_OPTIONS.clone();
      var coreOptions = options.get(CoreOptions.class);
      coreOptions.checkVisibility = checkVisibility;
      coreOptions.verboseVisibilityErrors = verboseVisibilityErrors;
      coreOptions.enforceTransitiveVisibility = enforceTransitiveVisibility;
      coreOptions.checkTestonlyForOutputFiles = checkTestonlyForOutputFiles;
      return options;
    }
  }

  // Reuse EMPTY_OPTIONS itself for the default flag values.
