...
class TargetCycleReporter extends AbstractLabelCycleReporter {
  ...
  public boolean maybeReportCycle(
    ...
    if (Iterables.any(
        ...
        key -> key instanceof GenAnalysisQueryKey)) {
      ...
      cycleInfo =
          CycleInfo.createCycleInfo(
              Iterables.filter(
                  cycleInfo.getPathToCycle(), key -> !(key instanceof GenAnalysisQueryKey)),
              Iterables.filter(cycleInfo.getCycle(), key -> !(key instanceof GenAnalysisQueryKey)));
    }
