...
public class ToolchainDeclarationsFunction implements SkyFunction {
  ...
  public SkyValue compute(SkyKey skyKey, Environment env)
    ...
    toolchainLabels =
        RegisteredToolchainsFunction.filterToolchainTypes(
            env, toolchainLabels, key.toolchainType());
    if (toolchainLabels == null) {
      return null;
    }
    // Retain all target-platform rejections when resolution debugging is enabled.
    if (!key.debug()) {
      toolchainLabels =
          RegisteredToolchainsFunction.filterTargetPlatforms(
              env, key.targetPlatformKey(), toolchainLabels, key.noConfigOptions());
      if (toolchainLabels == null) {
        return null;
      }
    }
    ...
    Map<Label, DeclaredToolchainInfo> infos =
        analyzeWithoutConfiguration(env, configIndependentLabels, key.noConfigOptions());
    if (infos == null) {
      return null;
    }
  ...
  private static Map<Label, DeclaredToolchainInfo> analyzeWithoutConfiguration(
      Environment env, ImmutableSet<Label> labels, BuildOptions noConfigOptions)
      throws InterruptedException, RegisteredToolchainsFunctionException {
    BuildConfigurationKey noConfig = BuildConfigurationKey.create(noConfigOptions);
